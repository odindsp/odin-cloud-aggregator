...
/**
 * 全局异常处理类：针对所有Controller抛出的异常进行最后的处理。 <ul> <li>如果抛出的是自定义异常，则使用自定义异常类中的属性，包装一个通用错误对象（包含一个整型的code和一个字符串类型的msg）。</li>
 * <li>如果抛出的是除自定义异常以外的异常，则统一使用HttpStatus Code 500作为响应码。</li> </ul>
 *
 * @author ningyu
 */
@ControllerAdvice
...
public class GlobalExceptionHandlerConfig {
    ...
    public BaseResponse handleException(Exception exception, HttpServletResponse response) {
        if (exception instanceof BaseException) {
            ...
            } else if (FileSizeLimitExceededException.class.isInstance(exception)) {
                log.warn("org.apache.tomcat.util.http.fileupload.FileUploadBase$FileSizeLimitExceededException");

                int httpStatusCode = HttpStatus.BAD_REQUEST.value();
                int bizStatusCode = 1;
                String bizStatusMsg = "上传文件大小超出限制，最大为" + maxFileSize;
    ...
    private List<BaseArgumentValidation> getBindResultErrors(Exception exception) {
        ...
        for (FieldError error : bindingResult.getFieldErrors()) {
            BaseArgumentValidation invalidArgument = new BaseArgumentValidation(error.getField(), error.getRejectedValue(),
